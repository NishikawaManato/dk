...
def noisesize1(backmask,noiseshape):                                 #処理が遅すぎる！！！！！
    ...
    for v1 in range(noiseshape.shape[0]):
        for v2 in range(noiseshape.shape[1]):
            if noiseshape[v1][v2]!=0:
                mask_size=np.where(mask_size==0,maskshift(v1,v2,backmask),mask_size)
    return mask_size
...
def GaussrainSet(h,w,count,mean,sigma):
    ...
    if mean==0:
        ...
        noisecount=int(count/(1-(2*norm.cdf(mean-cut_wide,mean,sigma))))
    ...
    drops[2] = np.random.normal(mean, sigma, noisecount)  # Random speeds
    drops[2] = np.where(drops[2]<mean-cut_wide,0,drops[2])
    drops[2] = np.where(drops[2]>mean+cut_wide,0,drops[2])
